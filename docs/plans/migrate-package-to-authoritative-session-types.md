...
- Update `test/fixtures/workspaces/contract.json` and `script/test` to assert the new Hub API, field names, scope, fixture directories, and docs. Add a scoped forbidden-token audit across active files for `session_templates`, `session_template_`, `template_id`, and `.botster/session-templates.json`; exclude historical plans/reports and only the exact allowlisted `OBSOLETE_FIELDS` record keys. Assert those three legacy keys remain present. Add a Lua regression that calls `create_workspace` separately with `default_session_template`, `default_session_template_id`, and `default_session_template_refs`, requiring the typed code `obsolete_field` rather than `unknown_field`; this test must fail if any key is removed from `OBSOLETE_FIELDS`. Keep the existing persisted-state `legacy_workspace_schema` coverage unchanged because that path is enforced independently by `validate_state` / `WORKSPACE_KEYS`.
...
2. Cold-cut negative proof: show that a `template_id` Workspaces spawn request returns `unknown_field`, makes zero session-type capability calls, and records no membership; show that an old `session_templates` fixture is rejected by the protocol-6 Hub rather than admitted as an alias. Separately, prove each frozen create argument (`default_session_template`, `default_session_template_id`, and `default_session_template_refs`) returns `obsolete_field`, explicitly not `unknown_field`, while the existing persisted-state `legacy_workspace_schema` regression remains green.
