...
- In the isolated real-Hub flow, add negative admission/request checks proving that neither an old `session_templates` manifest nor an old `template_id` request is silently translated. **Amended at Implement (see “Implement-stage plan corrections”): the manifest case is proven as *contributes nothing*, not as *rejected*.**
- Assert the published `botster_workspaces.spawn` MCP descriptor through real Hub discovery, not only the in-memory registered spec: exact description, object schema, property set and types, required set, `additionalProperties: false`, and absence of `template_id`.
...
2. **Cold-cut negative proof.** A `template_id` Workspaces spawn request returns `unknown_field`, makes zero session-type capability calls, and records no membership. A package whose manifest still declares `session_templates` is **admitted and enabled by the protocol-6 Hub but contributes zero effective session types**, so its id never reaches the selector and a spawn keyed on it returns typed `unknown_session_type` with no membership change. (Corrected at Implement: the Hub deserializes `LocalPackageManifest` with `#[serde(flatten)]` and `#[serde(default)] session_types` and no `deny_unknown_fields`, so the superseded key is ignored, not rejected. Parse-time rejection would be a Hub-owned schema change and is out of this repository's ownership.) Separately, each frozen create argument (`default_session_template`, `default_session_template_id`, `default_session_template_refs`) returns `obsolete_field`, explicitly not `unknown_field`, while the existing persisted-state `legacy_workspace_schema` regression stays green. Each negative assertion is demonstrated red against pre-change behavior, and the legacy-manifest control is additionally shown non-vacuous by proving the same fixture *does* contribute a session type when switched to the current key.
...
4. **Real package path.** With `BOTSTER_HUB_BIN` and `BOTSTER_SESSION_WORKER_BIN` built from a clean checkout at Hub `8a60bd5`, `script/test-hub-flow` starts one fresh Hub, admits the Git target, installs and enables the renamed session-type fixture, the legacy-manifest negative control, and this package, renders the target-first surface, lists effective session types through the real worker capability table, submits the returned fully qualified `session_type_id`, creates or reuses the managed worktree as expected, spawns, and appends exactly the returned `result.session_id` only after success. Package enable succeeding is itself proof that the scope migration is correct, since Hub `8a60bd58` hard-denies the legacy scope. The same run asserts the published `botster_workspaces.spawn` MCP descriptor discovered from the Hub, because the descriptor — not the handler — is the downstream agent contract.
...
## Implement-stage plan corrections

Accepted deviations are folded back into the decisions, scope, affected files, and acceptance checks above rather than left only in the implementation report, per [[implementation deviations must resync committed plan acceptance checks]]. Recorded after Review `review_1786040646_461844`.

1. **Gate 2's “rejected” was wrong about the Hub.** The plan asserted a legacy `session_templates` manifest is rejected. Hub `8a60bd58` `src/packages.rs:2857-2865` deserializes `LocalPackageManifest` with `#[serde(flatten)]` and `#[serde(default)] session_types` and no `deny_unknown_fields`, so the superseded key is silently ignored. The real contract is *contributes nothing*: the package installs and enables, offers no effective session type, and a spawn keyed on its id returns `unknown_session_type`. Gate 2 and implementation step 4 now say that. The first Implement pass reported gate 2 green while proving only the request half; the manifest half is now proven by `test/fixtures/legacy-session-template-manifest/` driven through `script/test-hub-flow`.
2. **Published MCP descriptor proof added.** The plan proved the migrated request contract only through the handler and the in-memory registered spec. `botster_workspaces.spawn`'s published descriptor is the downstream agent contract, so `script/hub_acceptance_smoke` now asserts it from real Hub discovery. This immediately caught real drift: the Hub serializes `McpToolDescriptor` with `rename_all = "camelCase"`, so the published key is `inputSchema` while the Lua spec authors `input_schema`.
3. **Cold-cut audit allowlist.** Beyond the three exact `OBSOLETE_FIELDS` keys, the audit skips lines carrying the literal marker `cold-cut negative control`, because gate 2 requires naming the superseded token inside audited files. `script/test` is excluded from the audit because it defines the forbidden literals; the legacy-manifest fixture is excluded because it *is* the negative control, and is instead pinned by assertions requiring it to keep the superseded key and never declare the current one.
4. **`script/shared_stack_acceptance` session-type id is fully qualified.** `SESSION_TYPE_ID` is `workspaces-shared-stack/shared-stack`, not the bare `shared-stack`. Effective rows have always been qualified `<source name>/<id>`, so the bare value could never match a rendered option; that lane has never executed, so this was latent rather than a regression.
5. **Scope additions.** One Lua action-adapter test covering the authored form-node-id to field seam, and `script/test-hub-flow` added to the cold-cut audit file list (but not to the leak-pattern scan, which it would fail on its `@example.invalid` Git fixture address).
