...
4. Build or identify Hub `fab44c5` and its matching `botster-session-worker`, then run the existing immutable-input shared-stack harness with the committed child checkout supplied as `workspaces_package`. Preserve `13924bbc3cf835835732d1d6f4a0e90167daa8dd` as the original failing baseline; implementation completed the compatible unchanged collision oracle at `44e6f767a19404cb473d6c643404044e13fcebec`, and fresh Verify completed it again from clean provenance-ledger revision `0ca3844bd72c17db9b3861bba6bed77e203ddbcd`. Do not edit `collision_proof!` or reduce its assertions. Capture the resulting summary/evidence ledger showing the harness revision, clean status, both exact collision identities, and zero state/resource mutation.
...
- Resolved proof lineage: `13924bbc3cf835835732d1d6f4a0e90167daa8dd` is the original failing baseline, `44e6f767a19404cb473d6c643404044e13fcebec` is the complete Implement proof, and clean revision `0ca3844bd72c17db9b3861bba6bed77e203ddbcd` is the fresh Verify provenance-ledger proof. Review confirmed `collision_proof!` is byte-identical between the baseline and Implement revisions, and Verify reran the same unchanged oracle. Evidence records the harness revision and clean status alongside Hub, locked Core/worker, Workspaces, Web, TUI, TUI-kit, and UI-contract revisions; mutable branch names or dirty checkouts do not count.
...
- Use `script/test-hub-flow shared-stack run <immutable-input-manifest> <new-evidence-dir>` (or its direct `script/shared_stack_acceptance` equivalent) from clean parent harness revision `0ca3844bd72c17db9b3861bba6bed77e203ddbcd` (or a later explicitly recorded compatible revision preserving `collision_proof!`), with the committed child revision as `workspaces_package` and Hub `fab44c5` plus matching worker provenance. Retain `13924bbc3cf835835732d1d6f4a0e90167daa8dd` as the original failure baseline and `44e6f767a19404cb473d6c643404044e13fcebec` as the Implement proof revision in the evidence history.
